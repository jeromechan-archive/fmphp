	...
	class SmartTemplate
	{
		...
		*/
		function SmartTemplate ( $template_filename = '' )
		{
			...
			if( array_key_exists('constants',$_CONFIG) && is_array($_CONFIG['constants']) ){
				foreach($_CONFIG['constants'] as $k=>$v){
					$this ->add_value($k, $v);
				}
			}
		}
			...
			if ($this->reuse_code)
			{
				if (is_file($this->cpl_file))
				{
					if ($this->mtime($this->cpl_file) > $this->mtime($this->template_dir . $this->tpl_file))
					{
						...
						preg_match_all('#<!--\s+INCLUDE\s*(.+?)\s*-->#is',file_get_contents($this->template_dir . $this->tpl_file),$matches);
						if($matches[1]) {
							foreach ($matches[1] as $sub_file) { 
								if($this->mtime($this->cpl_file) <= $this->mtime($this->template_dir .$sub_file)) {  
									$compile_template = true;
									break;
								}
							}
